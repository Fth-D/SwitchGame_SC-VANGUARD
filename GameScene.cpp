...
//--------------------------------------------------------------------------------------//
// rank=0 なら最も近い敵、rank=1 なら2番目に近い敵……という意味。						//
// 該当する敵がいなければ nullptr を返す(例えば敵が2体しかいないのにrank=3を指定した場合)。//
//======================================================================================//
...
void Game::UpdateGame(float dt)		// ここにゲームシーン更新コードを書く
{
	...
	for (int i = (int)Straight.size() - 1; i >= 0; i--)
	{
		...
		Float3 bp = Laser->GetPosition();

		if (beam.isFollowingPlayer)
		{
			// ボタンを押している間：Y座標をプレイヤーの砲口に同期させる
			Float3 playerPos = Vanguard->GetPosition();
			bp.y = playerPos.y + Vanguard->GetLaserMuzzleOffsetY();
			bp.x += beam.laserVelocityX * dt;
		}
		else
		{
			// ボタンが離された後：通常の直進のみ(追従しない)
			bp.x += beam.laserVelocityX * dt;
			bp.y += beam.laserVelocityY * dt;
		}

		Laser->SetPosition(bp);
	...
	for (int i = (int)Chase.size() - 1; i >= 0; i--)
	{
		...
		else
		{
			...
			if (target != nullptr)	// 敵が見つかった場合のみ方向を調整する
			{
				...
				if (speed > 0.0f)
				{
					float homingSpeed = 3970.0f;	// ミサイルの飛行速度(固定)
