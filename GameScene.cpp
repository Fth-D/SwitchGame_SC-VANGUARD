...
float HP_BreatheTimer = 0.0f;		// ARMOR文字の呼吸・脈動エフェクト用タイマー（毎フレーム += dt）
float HP_BreatheSpeed = 7.5f;		// 安全状態（青）の呼吸速度
float HP_RedFlickerSpeed = 12.0f;	// 危険状態（赤）の点滅速度。大きいほど激しく点滅する

//======================================================================================//
// 常駐UIラベルのグリッチ演出用パラメータ												//
//======================================================================================//
float UI_GlitchTimer = 0.0f;		// 毎フレーム += dt
...
void Game::InitializeGame()			// ここにゲームシーン初期化コードを書く
{
	...
	HP_Bar_Layer1->GetSprite().LoadTexture("rom:/texture/UI/01 -- GameScene/HP_Bar_sheet2.tga");	//画像読み込み
	HP_Bar_Layer1->GetSprite().DivideAnimationCells(19, 3);	//テキスチャーが縦横何分割になっているか指定
	HP_Bar_Layer1->GetSprite().CreateAnimation("HP_Bar_Nominal", 0, 18);	//アニメーションを作成（０フレイムなので１コマ）
	HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Nominal");	//アニメーションを指定
	HP_Bar_Layer1->GetSprite().SetAnimationFrameTime("HP_Bar_Nominal", 3.0f);	//アニメーション１週に何フレーム

	HP_Bar_Layer1->GetSprite().CreateAnimation("HP_Bar_Caution", 19, 37);	//アニメーションを作成（０フレイムなので１コマ）
	HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Caution");	//アニメーションを指定
	HP_Bar_Layer1->GetSprite().SetAnimationFrameTime("HP_Bar_Caution", 3.0f);	//アニメーション１週に何フレーム

	HP_Bar_Layer1->GetSprite().CreateAnimation("HP_Bar_Danger", 38, 56);	//アニメーションを作成（０フレイムなので１コマ）
	HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Danger");	//アニメーションを指定
	HP_Bar_Layer1->GetSprite().SetAnimationFrameTime("HP_Bar_Danger", 3.0f);	//アニメーション１週に何フレーム
...
void Game::UpdateGame(float dt)		// ここにゲームシーン更新コードを書く
{
	...
	{
		PlayerCharacter::MechStatus status = Vanguard->GetMechStatus();
		...
		if (status == PlayerCharacter::MechStatus::Nominal)
		{
			HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Nominal");
		}
		else if (status == PlayerCharacter::MechStatus::Caution)
		{
			HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Caution");
		}
		else
		{
			HP_Bar_Layer1->GetSprite().SetAnimation("HP_Bar_Danger");
		}
...
void Game::DrawGame()				// ここにゲームシーン描画コードを書く
{
	...
	if (Vanguard != nullptr)
	{
		...
		Text::Draw("Armor", 182.5f, 42.5f, 40.0f);
		Text::SetColor(hpColor.x, hpColor.y, hpColor.z, hpAlpha);
		Text::Draw("Integrity", 182.5f, 75.5f, 40.0f);

		Text::SetColor(hpColor.x, hpColor.y, hpColor.z, hpAlpha);
		Text::Draw(hpText, 495.0f, 35.0f, 68.0f);

		Text::SetColor(hpColor.x, hpColor.y, hpColor.z, hpAlpha);
		Text::Draw("%", 627.5f, 35.0f, 68.0f);
	}


	if (Vanguard!=nullptr)
	{
		Text::SetColor(0.18f, 0.88f, 0.84f, 1.0f);	//
