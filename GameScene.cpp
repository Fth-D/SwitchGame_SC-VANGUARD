...
GameObject* HP_Bar_underLayer = nullptr;	// HP Bar

PlayerCharacter* Vanguard = nullptr;	//プレイヤー機体
GameObject* objB = nullptr;	//敵機体(テスト用)

GameObject* Background_Layer1 = nullptr;	//背景レイヤー1（遠景）
GameObject* Background_Layer2 = nullptr;	//背景レイヤー2（中景）
GameObject* Background_Layer3 = nullptr;	//背景レイヤー3（近景）

float HP_Leungh = 563.0f;
float HP_UnderLayerDisplayPrecent = 1.0f;
float HP_UnderLayerCatchUpSpeed = 2.5f;
...
void Game::InitializeGame()			// ここにゲームシーン初期化コードを書く
{
	...
	// --- 背景	--- //
	Background_Layer1 = AddObject(new GameObject);
	Background_Layer2 = AddObject(new GameObject);

	// --- プレイヤー機体の生成 --- //
	Vanguard = (PlayerCharacter*)AddObject(new PlayerCharacter); //プレイヤー機体を生成

	Background_Layer3 = AddObject(new GameObject);
	...
	UI_Frame->GetSprite().LoadTexture("rom:/texture/UI/GameScene/UI_Frame_Pixel.tga");	// 画像読み込み
	...
	HP_Bar->GetSprite().LoadTexture("rom:/texture/UI/GameScene/HP_Bar_sheet.tga");	//画像読み込み
	HP_Bar->GetSprite().DivideAnimationCells(19, 1);	//テキスチャーが縦横何分割になっているか指定
	HP_Bar->GetSprite().CreateAnimation("HP_Bar", 0, 18);	//アニメーションを作成（０フレイムなので１コマ）
	HP_Bar->GetSprite().SetAnimation("HP_Bar");	//アニメーションを指定
	HP_Bar->GetSprite().SetAnimationFrameTime("HP_Bar", 0.01f);	//アニメーション１週に何フレーム

	HP_Bar_underLayer = new GameObject;
	HP_Bar_underLayer->Activation();
	HP_Bar_underLayer->Show();
	HP_Bar_underLayer->GetSprite().Initialize();
	HP_Bar_underLayer->SetObjectType(UI);
	HP_Bar_underLayer->SetFixedScreenPositionState(true);
	HP_Bar_underLayer->SetPosition(MakeFloat3(371.0f, 152.5f, 0.0f));
	HP_Bar_underLayer->GetSprite().SetPolygonSize(MakeFloat2(HP_Leungh, 100.0f));
	HP_Bar_underLayer->GetSprite().LoadTexture("rom:/texture/UI/GameScene/HP_Bar_Layer2.tga");
	HP_Bar_underLayer->GetSprite().DivideAnimationCells(1, 1);
	HP_Bar_underLayer->GetSprite().CreateAnimation("HP_Bar_UnderLayer", 0, 0);
	HP_Bar_underLayer->GetSprite().SetAnimation("HP_Bar_UnderLayer");

	// --- Vanguard ---
	Vanguard->Setup(-430.0f, 0.0f);	//初期化（画像、当たり判定、位置）

	// --- Background_Layer1_far ---
	Background_Layer1->Activation();	//有効化
	Background_Layer1->Show();	//描画有効化
	Background_Layer1->GetSprite().Initialize();	//スプライト初期化
	Background_Layer1->SetObjectType(Other);	//オブジェクト分類
	Background_Layer1->SetPosition(MakeFloat3(0.0f, 0.0f, 0.0f));	//座標指定
	Background_Layer1->GetSprite().SetPolygonSize(MakeFloat2(11520.0f, 1080.0f));	//サイズ指定
	Background_Layer1->GetSprite().LoadTexture("rom:/texture/Background/Layer1_far.tga");	//画像読み込み
	Background_Layer1->GetSprite().DivideAnimationCells(1, 1);	//テキスチャーが縦横何分割になっているか指定
	Background_Layer1->GetSprite().CreateAnimation("Background_Layer1_far", 0, 0);	//アニメーションを作成（０フレイムなので１コマ）
	Background_Layer1->GetSprite().SetAnimation("Background_Layer1_far");	//アニメーションを指定

	// --- Background_Layer2_mid ---
	Background_Layer2->Activation();	//有効化
	Background_Layer2->Show();	//描画有効化
	Background_Layer2->GetSprite().Initialize();	//スプライト初期化
	Background_Layer2->SetObjectType(Other);	//オブジェクト分類
	Background_Layer2->SetPosition(MakeFloat3(0.0f, 0.0f, 0.0f));	//座標指定
	Background_Layer2->GetSprite().SetPolygonSize(MakeFloat2(11520.0f, 1080.0f));	//サイズ指定
	Background_Layer2->GetSprite().LoadTexture("rom:/texture/Background/Layer2_mid.tga");	//画像読み込み
	Background_Layer2->GetSprite().DivideAnimationCells(1, 1);	//テキスチャーが縦横何分割になっているか指定
	Background_Layer2->GetSprite().CreateAnimation("Background_Layer2_mid", 0, 0);	//アニメーションを作成（０フレイムなので１コマ）
	Background_Layer2->GetSprite().SetAnimation("Background_Layer2_mid");	//アニメーションを指定

	// --- Background_Layer3_near ---
	Background_Layer3->Activation();	//有効化
	Background_Layer3->Show();	//描画有効化
	Background_Layer3->GetSprite().Initialize();	//スプライト初期化
	Background_Layer3->SetObjectType(Other);	//オブジェクト分類
	Background_Layer3->SetPosition(MakeFloat3(0.0f, 0.0f, 0.0f));	//座標指定
	Background_Layer3->GetSprite().SetPolygonSize(MakeFloat2(11520.0f, 1080.0f));	//サイズ指定
	Background_Layer3->GetSprite().LoadTexture("rom:/texture/Background/Layer3_near.tga");	//画像読み込み
	Background_Layer3->GetSprite().DivideAnimationCells(1, 1);	//テキスチャーが縦横何分割になっているか指定
	Background_Layer3->GetSprite().CreateAnimation("Background_Layer3_near", 0, 0);	//アニメーションを作成（０フレイムなので１コマ）
	Background_Layer3->GetSprite().SetAnimation("Background_Layer3_near");	//アニメーションを指定
...
void Game::UpdateGame(float dt)		// ここにゲームシーン更新コードを書く
{
	...
	//	ＨＰゲージの更新																//
	//==================================================================================//
	if (HP_Bar != nullptr && Vanguard != nullptr)
	{
		float hpPercent = (float)Vanguard->GetHp() / (float)Vanguard->GetMaxHp();
		float currentWidth = HP_Leungh * hpPercent;

		HP_Bar->SetPosition(MakeFloat3(89.5f + currentWidth * 0.5f, 152.5f, 0.0f));	// 89.5 = 371.0 - 563.0/2（左端固定）
		HP_Bar->GetSprite().SetPolygonSize(MakeFloat2(currentWidth, 100.0f));
		HP_Bar->Update(dt);
	}

	if (HP_Bar_underLayer != nullptr && Vanguard != nullptr)
	{
		float hpPercent = (float)Vanguard->GetHp() / (float)Vanguard->GetMaxHp();
		HP_UnderLayerDisplayPrecent += (hpPercent - HP_UnderLayerDisplayPrecent) * HP_UnderLayerCatchUpSpeed * dt;
		float underLayerWidth = HP_Leungh * HP_UnderLayerDisplayPrecent;

		HP_Bar_underLayer->SetPosition(MakeFloat3(89.5f + underLayerWidth * 0.5f, 152.5f, 0.0f));
		HP_Bar_underLayer->GetSprite().SetPolygonSize(MakeFloat2(underLayerWidth, 100.0f));
		HP_Bar_underLayer->Update(dt);
	}
...
void Game::DrawGame()				// ここにゲームシーン描画コードを書く
{
	...
	if (HP_Bar_underLayer != nullptr)
	{
		HP_Bar_underLayer->Draw();
	}
