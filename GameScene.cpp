...
void Game::InitializeGame()			// ここにゲームシーン初期化コードを書く
{
	...
	HP_Bar->SetPosition(MakeFloat3(0.0f, 0.0f, 0.0f));	// 座標指定
	HP_Bar->GetSprite().SetPolygonSize(MakeFloat2(700.0f,950.0f));	//サイズ指定
	HP_Bar->GetSprite().LoadTexture("rom:/texture/UI/HP_Bar.tga");	//画像読み込み
	UI_Frame->GetSprite().DivideAnimationCells(1, 1);	//テキスチャーが縦横何分割になっているか指定
	UI_Frame->GetSprite().CreateAnimation("HP_Bar", 0, 0);	//アニメーションを作成（０フレイムなので１コマ）
	UI_Frame->GetSprite().SetAnimation("HP_Bar");	//アニメーションを指定
