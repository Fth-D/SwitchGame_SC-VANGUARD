...
void Game::InitializeGame()			// ここにゲームシーン初期化コードを書く
{
	...
	Background_Layer2_A->SetPosition(MakeFloat3(0.0f, 540.0f, 0.0f));	//座標指定
	...
	Background_Layer2_B->SetPosition(MakeFloat3(0.0f, 540.0f, 0.0f));	//座標指定
	...
	Background_Layer3_A->SetPosition(MakeFloat3(0.0f, 540.0f, 0.0f));	//座標指定
	...
	Background_Layer3_B->SetPosition(MakeFloat3(0.0f, 540.0f, 0.0f));	//座標指定
...
void Game::UpdateGame(float dt)		// ここにゲームシーン更新コードを書く
{
	...
	if (Background_Layer1_A != nullptr && Background_Layer1_B != nullptr)
	{
		...
		posA.x -= bg_ScrollSpeed * dt;
		posB.x -= bg_ScrollSpeed * dt;

		// Aが完全に画面左に消えたら、Ｂのすぐみぎに直す（回収作業）
		if (posA.x+bg_TextureHight*0.5f<0.0f)
		{
